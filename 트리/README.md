...
### Heap의 구현 ☆★☆
● 최소 힙 등을 구현할 때 Linked list보다 Array list로 하는 이유(SWEA_5177 관련).     
  
  → 내부 모듈 없이 따로 구현할 때, 두 방법 모두 **새로운 노드를 마지막 위치에 삽입** 한다는 힙의 로직은 구현할 수 있으나 실질적으로 코드로 만들기에는 Array list가 효율적이기 때문.  
  
  → PS 문제마다 입력 조건이 다르겠지만, Linked List로 구현된 트리에서 따로 명시해주지 않는이상 node 추가시 마지막 위치에 추가해야하는데 그 위치를 알기 어렵다.  
  
  → ArrayList(list in python)에서는 **간단하게 Slicing** 할 수 있어 접근하기가 용이하고, Data Set이 크지 않은 이상 **Linked List 대비 시간복잡도도 더 효율적** 이다.  
