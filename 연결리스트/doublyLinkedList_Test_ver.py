...
# Opentutorials의 이중연결리스트-java 구현 코드를 python으로 자체 변환. (230413)
...
lst.addtoLast(3)
lst.addtoFirst(4)
print(f'add 전 = {lst.toString()}')
print(f'index(3) 노드 찾기 = {lst.node(3).data}')
lst.add(3, 5)
print(f'add(3, 5) 후 = {lst.toString()}')
lst.remove(3)
print(f'remove(3) 후 = {lst.toString()}')
lst.removeFirst()
print(f'removeFirst() 후 = {lst.toString()}')
lst.removeLast()
print(f'removeLast() 후 = {lst.toString()}')
print(f'get(0) = {lst.get(0)}')

# get() = index로 data로 조회 / indexOf() = data로 index 조회
print(f'indexOf(99) = {lst.indexOf(99)}')
print(f'< 최종 lst > = {lst.toString()}')
'''

'''
1. size()           : 사이즈
2. addtoFirst(data) : 맨 앞에 추가
3. addtoLast(data)  : 맨 뒤에 추가
4. node(idx)        : 특정 노드 객체 찾기 
5. add(idx, data)   : 특정 위치에 노드 추가
6. toString()       : 전체 노드값 출력
7. removeFirst()    : 맨 앞 노드 삭제
8. remove(idx)      : 특정 노드 삭제
9. removeLast()     : 맨 뒤 노드 삭제
10. get(idx)        : index로 data 조회
11. indexOf(data)   : data로 index 조회
'''
