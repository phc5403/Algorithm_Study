...
for tc in range(1, int(input()) + 1):
    ...
    for _ in range(K):
        ...
        # 1. M칸 이동 후 = size 범위 내.
        if 0 < cur <= lst.size - 1:
            lst.add(cur, lst.get(cur) + lst.get(cur - 1))

        # 2. M칸 이동 후 마지막 위치 = 뒤로 밀려난 칸이 없음 = 시작 숫자와 앞숫자 더함.
        elif cur == lst.size:
            lst.addtoLast(lst.tail.data + lst.head.data)

        # 3. 이동 후가 size 넘어갈 시 처음부터 이어서.
        elif cur > lst.size:
            cur -= lst.size
            lst.add(cur, lst.get(cur) + lst.get(cur - 1))

    # print(f'최종 List = {lst.toString()}, {lst.size}')
    ...
    while res is not None and cnt < 10:
